...
def test_synthetic_mft_analysis(analyzed_output):
    with open(analyzed_output, 'r', newline='') as f:
        ...
        records = list(csv_reader)    assert len(records) == 1000, f"Expected 1000 records, but got {len(records)}"    file_types = defaultdict(int)
    ...
    for record in records:        record_statuses[record['Record Status']] += 1
        file_types[record['File Type']] += 1        unique_parent_records.add(record['Parent Record Number'])        if record['Filename']:
            ext = os.path.splitext(record['Filename'])[1].lower()
            filename_extensions[ext] += 1        for time_field in ['SI Creation Time', 'FN Creation Time']:
            if record[time_field] and record[time_field] != "Not defined":
                ...
                time_ranges[field_key]['max'] = max(time_ranges[field_key]['max'], dt)        assert record['Has Standard Information'] in ['True', 'False']
        assert record['Has File Name'] in ['True', 'False']        if record['File Type'] == 'File':
            assert record['Has Data'] == 'True', f"File record {record['Record Number']} missing Data attribute"        assert record['Sequence Number'].isdigit(), f"Invalid Sequence Number: {record['Sequence Number']}"    assert record_statuses['Valid'] == 1000, f"Expected all records to be Valid, but got {record_statuses['Valid']}"
    ...
    assert len(unique_parent_records) > 1, "Expected multiple parent records"    assert filename_extensions['.txt'] > 0, "Expected some .txt files"
    assert filename_extensions['.exe'] > 0, "Expected some .exe files"
    assert filename_extensions['.dll'] > 0, "Expected some .dll files"    for time_field, range_data in time_ranges.items():
        assert range_data['min'] < range_data['max'], f"Invalid time range for {time_field}"
        assert (range_data['max'] - range_data['min']).days < 365 * 10, f"Time range for {time_field} exceeds 10 years"    assert any(record['Filename'] == 'MFT' for record in records), "MFT file not found"
...
def test_synthetic_mft_statistics(synthetic_mft):
    ...
    asyncio.run(analyzer.analyze())    assert analyzer.stats['total_records'] == 1000
